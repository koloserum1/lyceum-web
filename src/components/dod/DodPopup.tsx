  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/dod-lead", {
        ...
        body: JSON.stringify({ email, website, source: "dod-popup" }),
      });
        ...
        {sent ? (
          ...
          <form className="mt-6 space-y-4" onSubmit={onSubmit} noValidate>
            {ctaHint ? (
              ...
              <p className="m-0 text-sm leading-relaxed text-brand-fg3">
                Zadaj svoj e-mail a my sa ti ozveme.
              </p>
            )}
            <input type="text" name="website" className="hidden" tabIndex={-1} autoComplete="off" aria-hidden />
            <div>
              <label htmlFor={emailFieldId} className="mb-1 block text-sm font-bold text-brand-fg1">
